...
| **スマホ版（軽量検索特化）**<br>`hololive-shop-smartphone-search.user.js` | Userscriptsアプリ（無料）<br> Firefox for Android        | ・iOS Safari、Firefox for Androidで完全動作<br>・全ページ自動取得＆ローカル保存<br>・タレント名＋フリーワード検索<br>・新着順／古い順ソート<br>・キーワード自由追加 |
...
- ときのそら〜DEV_ISまで初期データ入り（海外、IDまで入れるとコードが長くなるので各自で追加してください）
